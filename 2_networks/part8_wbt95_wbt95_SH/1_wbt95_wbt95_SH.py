...
for iyear in range(0,42):
  ...
  for alat in range(np.shape(wbt95)[1]):
    ...
    for alon in range(np.shape(wbt95)[2]):
      ...
      concurrent_days = np.sum(t_True_3D[:,:,:]+wbt95[:,:,:]==2,axis=0)
  ...
  ds0.to_netcdf("/public/home/fcai/extreme5_degree/25_humid_heat/2_network/part8_wbt95_wbt95_SH/wbt95_wbt95_SH2x2_"+str(iyear+1982)+".nc")
  ds0.close()
